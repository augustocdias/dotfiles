...
--
-- Reads from the root flake.lock to get current pinned revisions,
...
-- Config
local NEOVIM_FLAKE_PATH = vim.fn.expand('~/nixos')
...
-- Only inputs with the nvim- prefix are neovim plugins.
-- This is consistent with how update-neovim-plugins discovers plugins.
...
local function fetch_all()
    ...
    if not plugins then
        vim.notify('Failed to read flake.lock', vim.log.levels.ERROR)
        return
    end
