...
set json_file "$HOME/nixos/modules/programs/thunderbird/extensions.json"
...
set extensions (jq -r 'keys[]' $json_file | sort); or begin
    echo -e $RED"Failed to parse $json_file"$NC
    exit 1
end
...
cp $json_file $temp_json; or exit 1
for ext in $extensions
    ...
    jq ".\"$ext\".url = \"$ext_url\" | .\"$ext\".addonId = \"$ext_id\" | .\"$ext\".sha256 = \"$ext_sha\"" $temp_json >$temp_dir/tmp.json; or begin
        echo -e $RED"Failed to update JSON for $ext"$NC
        exit 1
    end
    mv $temp_dir/tmp.json $temp_json; or exit 1
end
mv $temp_json $json_file; or exit 1

echo -e $GREEN"Done!"$NC

if test $rebuild = true
    sudo nixos-rebuild switch --flake ~/nixos#laptop; or exit 1
else
    echo "Run: sudo nixos-rebuild switch --flake ~/nixos#laptop"
end
